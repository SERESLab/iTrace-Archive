...
import edu.ysu.itrace.WindowTracker;
...
public class ITrace extends AbstractUIPlugin implements EventHandler {
	...
	private Queue<Shell> windowList = new LinkedList<Shell>();
	...
	public ITrace() {  
		/*
		 * if(editorPart != null){ StyledText styledText = (StyledText)
		 * editorPart.getAdapter(Control.class); if(styledText != null){
		 * ITextOperationTarget t = (ITextOperationTarget)
		 * activeEditor.getAdapter(ITextOperationTarget.class); if(t instanceof
		 * ProjectionViewer){ ProjectionViewer projectionViewer =
		 * (ProjectionViewer)t; tokenHighlighters.put(activeEditor, new
		 * TokenHighlighter(styledText, showTokenHighlights, projectionViewer));
		 * } } }
		 */
	...
	public void start(BundleContext context) throws Exception {
		...
		EyeTrackerFactory.setTrackerType(
				EyeTrackerFactory.TrackerType.valueOf(prefStore.getString(PluginPreferences.EYE_TRACKER_TYPE)));
	...
	public void removeWindow() {
		for(Shell shell : windowList) {
			// Find removed shell.
			if (shell.isDisposed())
				windowList.remove(shell);
		}
	}
	...
	public void configureSessionInfo() {
		sessionInfo.config();
		if (sessionInfo.isConfigured()) {
			xmlSolver.config(sessionInfo.getSessionID(), sessionInfo.getDevUsername());
			jsonSolver.config(sessionInfo.getSessionID(), sessionInfo.getDevUsername());
		}
	}
		...
		if (!tokenHighlighters.containsKey(editorPart)) {
			...
			if (styledText != null) {
				...
				if (t instanceof ProjectionViewer) {
					...
					tokenHighlighters.put(activeEditor,
							new TokenHighlighter(styledText, showTokenHighlights, projectionViewer));
				}
			}
		}
		...
		while (!childrenQueue.isEmpty()) {
			for (Control child : childrenQueue.remove()) {
				...
				if (childScreenBounds.contains(screenX, screenY)) {
					...
					IGazeHandler handler = (IGazeHandler) child.getData(HandlerBindManager.KEY_HANDLER);
					if (child.isVisible() && handler != null) {			
						if(!windowList.isEmpty()) {
							// If there are windows, check that the gaze isn't falling on a pop up.
							for(Shell shell : windowList) {
								Rectangle pos = shell.getBounds();
								if (pos.contains(screenX, screenY))
									return null;
							}		
						}
								
						return handler.handleGaze(screenX, screenY, screenX - childScreenBounds.x,
		...
		if (event.getTopic() == "iTrace/newgaze") {
			...
			if (g != null) {
				if (!rootShell.isDisposed()) {
					...
					Queue<Control[]> childrenQueue = new LinkedList<Control[]>();
					if(current != null) {
						childrenQueue.add(current.getChildren());
					}
					...
					if (compositeCount == 1 && !windowList.contains(current)) {
						current.addShellListener(new WindowTracker(current, current.getText()));
					...
					if (response != null) {
						if (recording) {
							...
							if (jsonOutput)
								eventBroker.post("iTrace/jsonOutput", response);
						}
