...
/**
 * Implements the gaze handler interface for a FigureCanvas widget of a UML diagram
 * editor window.
 */
public class UmlFigureCanvasGazeHandler implements IGazeHandler {
	 ...
	 */
	public UmlFigureCanvasGazeHandler(Object target, IWorkbenchPartReference partRef){
	...
	public IGazeResponse handleGaze(final int x, final int y, final Gaze gaze) {
		...
		IGazeResponse response = new IGazeResponse(){
			...
			{
				Rectangle viewport = new Rectangle(canvas.getViewport().getViewLocation(),
					...
					for(Object child : figures.remove()){
						if(child instanceof IFigure){
							...
							if(viewport.intersects(childBounds)){
								...
								if(childBounds.contains(x + viewport.x, y + viewport.y)){
									if(childFig instanceof ClassFigure){
										
										ClassFigure classFig = (ClassFigure)childFig;
										int localX = x + viewport.x - classFig.getLocation().x;
										int localY = y + viewport.y - classFig.getLocation().y;
										
										this.properties.put("class_name", classFig.getNameString());
										
										int index = classFig.getPropertyIndex(localX, localY);
										if (index >= 0){
											this.properties.put("attribute", classFig.getPropertyString(index));
										}
										else{
											index = classFig.getOperationIndex(localX, localY);
											if (index >= 0){
												this.properties.put("method", classFig.getOperationString(index));
											}
										}
